...
import Helpers from "@app/src/lib/helpers";
...
export default class Controller {
  ...
  async _create() {
    try {
      if (!this.tableName)
        return [new Error("tableName : table name must be defined!"), null];
      if (
      ...
      const response = await this.prisma[this.tableName].create({
        data: this.fields,
      });
  ...
  async _detail() {
    try {
      ...
      const response = await this.prisma[this.tableName].findFirst(condition);
